...
"""
This module contains the parent class (called BaseModel) to take care of the initialization,
serialization and deserialization of the future instances.
...
class BaseModel:
    ...
    def __init__(self, *args, **kwargs):
        ...
        Args:
          ...
          created_at (datetime): initialized with the current datetime when an instance is created.
          updated_at (daytime): initialized with the current datetime
          when an instance is created and it will be updated every time the object is changed.
        """
        if kwargs:
            if kwargs["__class__"] == self.__class__.__name__:
                    self.id, self.created_at,
                    self.updated_at, *_ = kwargs.values()
